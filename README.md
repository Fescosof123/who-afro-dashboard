...
- Public online data ingestion from:
  ...
  - Copernicus CEMS Flood / GloFAS posture checks (hydrological source availability and access-state monitoring)
...
- `POST /api/country-feed`
  - Ingests country-level override data from Power Automate or another scheduler.
  - Accepts either CSV text body or JSON body (`[{...}]` or `{ "records": [{...}] }`).
  - Optional auth header: `x-dashboard-ingest-token` (required when `COUNTRY_FEED_INGEST_TOKEN` is set).
  - Automatically detects the WHO `FCV-Services-Deliveries-Database` export schema and aggregates district rows into country-month summaries.
- `GET /api/country-feed/status`
  - Returns ingest freshness, row counts, and last sync status.
...
- `COUNTRY_FEED_INGEST_TOKEN` (recommended)
  - Shared secret required by `POST /api/country-feed` via `x-dashboard-ingest-token` header.
- `COUNTRY_FEED_PULL_MODE=auto|off` (default `auto`)
  - Enables periodic pull from an automated source.
- `COUNTRY_FEED_LOCAL_FILE` (default `dashboard-app/FCV-Services-Deliveries-Database.csv`)
  - Local synced CSV file path used in auto mode when available (recommended for OneDrive/SharePoint sync workflows).
  - If this file exists, it is prioritized over `COUNTRY_FEED_CSV_URL`.
- `COUNTRY_FEED_CSV_URL` (optional)
  - Public or tokenized CSV URL for automated pull (for example, SharePoint CSV export link).
- `COUNTRY_FEED_REFRESH_MINUTES` (default `60`)
  - Pull cadence for remote CSV in `auto` mode.
- `COUNTRY_FEED_MAX_STALE_MINUTES` (default `180`)
  - Freshness threshold used in `country_feed_status`.

Copernicus flood note:

- `/api/dashboard-data` now also exposes `cems_flood_source_status`.
- This is a posture-check object, not a live flood-event feed.
- It is intended to show whether Copernicus CEMS Flood / GloFAS documentation and access pathways are reachable, and whether the visible portal appears credential-gated.
- Until a stable authenticated or public machine-readable connector is added, visible flood events should still be interpreted as GDACS + ReliefWeb driven.
...
- Country feed pushes are cached in `dashboard-app/data/country-feed-cache.json` and merged into country values before risk scoring.
...
## Country Feed Template

- Use `dashboard-app/data/country-feed-template.csv` as the source template for Power Automate CSV export or manual validation.
- Required column: `iso3`.
- Supported columns:
...
## FCV Services Deliveries Integration

- The dashboard can ingest the SharePoint Excel export already present in `FCV-Services-Deliveries-Database` format without manual reshaping.
- In `auto` mode, if `FCV-Services-Deliveries-Database.csv` is synced into `dashboard-app/`, the server will ingest it automatically during dashboard refresh (no manual POST required).
- If Power Automate posts rows from that workbook, the server detects the schema and produces:
  - `service_delivery_status`
  - `service_delivery_by_country`
  - `countries[].service_delivery`
- Aggregation rule:
  - Count fields are summed by country and month.
  - Rate and percentage fields are averaged by country and month as an interim operational rule.
...
- Copernicus CEMS Flood / GloFAS is currently integrated only as an access-state and documentation monitor; it is not yet a direct real-time hydrological ingestion pipeline.
