...
const RISK_BANDS = [
  { min: 0, max: 20, label: "Band 1 Low", color: "#7bc8ff" },
  { min: 21, max: 40, label: "Band 2 Guarded", color: "#37a2df" },
  { min: 41, max: 60, label: "Band 3 Elevated", color: "#f5b255" },
  { min: 61, max: 80, label: "Band 4 High", color: "#ef7f37" },
  { min: 81, max: 100, label: "Band 5 Critical", color: "#cf3c3c" }
];
...
const els = {
  ...
  exportValidationBtn: document.getElementById("exportValidationBtn"),
  validationExportNote: document.getElementById("validationExportNote"),
  validationExportLast: document.getElementById("validationExportLast"),
...
function formatIssueCode(value) {
  ...
  if (Number.isNaN(d.getTime())) {
    return "WHO-AFRO-HIB-UNKNOWN";
  }
  const year = d.getUTCFullYear();
  const month = String(d.getUTCMonth() + 1).padStart(2, "0");
  const day = String(d.getUTCDate()).padStart(2, "0");
  return `WHO-AFRO-HIB-${year}${month}${day}`;
}
...
function renderCaveat(pageId) {
  ...
  els.caveatLine.textContent = PAGE_CAVEATS[pageId] || "";
}
    ...
    const signalMarkup = countrySignals.length
      ? countrySignals.map((item) => `
          <div class="feed-item conflict-feed-item">
            ...
            <div class="signal-summary">${item.summary || "No summary was available from the source item."}</div>
          </div>
        `).join("")
...
function recommendationCard(rec) {
  return `
    <div class="rec-item rec-${rec.priority}">
      <div class="rec-priority-label">${rec.priority.toUpperCase()}</div>
...
function stopAutoRotate(reason = "") {
  ...
  els.autoRotateBtn.textContent = "Start Auto Display";
  setAutoStatus(reason ? `Auto display paused: ${reason}` : "Auto display is off.");
}
...
function queueNextAutoRotate() {
  if (!autoRotateTimer && autoRotateTimer !== null) {
    return;
  }
  ...
  autoRotateTimer = setTimeout(() => {
    ...
    if (els.autoRotateBtn.textContent === "Stop Auto Display") {
      queueNextAutoRotate();
    }
  }, delay);
}
...
function startAutoRotate() {
  ...
  els.autoRotateBtn.textContent = "Stop Auto Display";
  setAutoStatus(`Auto display is on (${Math.round(interval / 1000)} sec base, page-aware timing).`);
}

function setCurrentDate() {
  const now = new Date();
  const opts = { weekday: "short", year: "numeric", month: "short", day: "numeric" };
  els.currentDate.textContent = `Date: ${now.toLocaleDateString("en-GB", opts)}`;
}

function buildBandLegend() {
  els.bandLegend.innerHTML = RISK_BANDS.map((b) => `<div class="band-item" style="background:${b.color}">${b.label}</div>`).join("");
}
...
async function exportValidationSnapshot() {
  if (!dashboardState) {
    showExportStatus("Export skipped: dashboard data is not loaded yet.", "error", 5000);
    return;
  }

  let ledger = dashboardState.metric_ledger || null;
  if (!ledger) {
    try {
      const response = await fetch("/api/metric-ledger", { cache: "no-store" });
      if (response.ok) {
  ...
  if (!ledger) {
    showExportStatus("Export skipped: metric ledger is not available in this refresh.", "error", 6000);
    return;
  }

  const stamp = (dashboardState.generated_at || new Date().toISOString()).replace(/[:.]/g, "-");
  const ruleVersion = String(ledger.rule_version || "metric-ledger-v1").replace(/[^a-z0-9-]+/gi, "-").toLowerCase();
  const issueCode = formatIssueCode(dashboardState.generated_at);
  const snapshot = {
    exported_at: new Date().toISOString(),
    dashboard_generated_at: dashboardState.generated_at || null,
    dashboard_issue_code: issueCode,
    scope: dashboardState.scope || null,
    metric_ledger: ledger,
    source_status: {
      reliefweb_api_status: dashboardState.reliefweb_api_status || null,
      cems_flood_source_status: dashboardState.cems_flood_source_status || null,
      cyclone_source_status: dashboardState.cyclone_source_status || null,
      who_don_source_status: dashboardState.who_don_source_status || null,
      iom_dtm_source_status: dashboardState.iom_dtm_source_status || null,
      acaps_source_status: dashboardState.acaps_source_status || null,
      acled_source_status: dashboardState.acled_source_status || null,
      fews_source_status: dashboardState.fews_source_status || null,
      fews_ipc_source_status: dashboardState.fews_ipc_source_status || null,
      ipc_source_status: dashboardState.ipc_source_status || null
    },
    source_freshness: dashboardState.source_freshness || null
  };

  const fileName = `validation-snapshot-${issueCode}-${ruleVersion}-${stamp}.json`;
  downloadJsonSnapshot(fileName, snapshot);
  if (els.validationExportLast) {
    els.validationExportLast.textContent = `Last validation export: ${fileName} at ${formatDateTime(snapshot.exported_at)}.`;
  }
  showExportStatus(`Validation snapshot exported: ${fileName}`, "success", 4500);
}

function renderValidationExportNote() {
  if (!els.validationExportNote) {
    return;
  }
  const ruleVersion = dashboardState?.metric_ledger?.rule_version || "metric-ledger-v1";
  const issueCode = formatIssueCode(dashboardState?.generated_at);
  els.validationExportNote.textContent = `Validation snapshot downloads to the default browser downloads folder. Filename includes issue code ${issueCode} and rule version ${ruleVersion}.`;
}
...
function renderForecast(countryIso3) {
  ...
  els.forecastInsights.innerHTML = `
    ...
    <p><strong>Regional source load:</strong> ICPAC bulletins ${forecasts.length}, drought signals ${droughtSignals.length}, cyclone projection signals ${cycloneSignals.length}.</p>
...
function renderWhoDonAlerts() {
  ...
  els.whoDonFeed.innerHTML = items.length
    ...
    : "No AFRO-mapped WHO DON outbreak alert was found in the current 30-day window.";
}
...
function renderHazardSourceSummaries() {
  ...
  if (els.gdacsSummary) {
    const floodEvents = gdacs.by_type?.Flood || 0;
    ...
    els.gdacsSummary.innerHTML = `
      ...
      <p>Flood events: <strong>${floodEvents}</strong> | AFRO-relevant cyclone events: <strong>${cycloneEvents}</strong></p>
...
function renderIcpacForecasts() {
  const forecasts = dashboardState.forecasts || [];
  if (!forecasts.length) {
    els.icpacFeed.innerHTML = "No ICPAC products were available during this pull.";
    return;
  }
...
function renderOperationalReportPage() {
  ...
  const conflictInsertCountries = countries
    ...
    .slice(0, 6);
  const issueCode = formatIssueCode(dashboardState.generated_at);
  const nextUpdate = humanAgeFromIso(dashboardState.generated_at) === "n/a" ? "next refresh cycle" : "next automated refresh cycle";
  ...
  const coverQualityLabel = coverQualitySeverity === "bad" ? "Action needed" : coverQualitySeverity === "warn" ? "Monitor" : "Good";
  ...
  const bulletinHighlights = [
    {
      ...
      body: `Highest immediate composite pressure is concentrated in ${highestRiskCountry}, with the current priority watchlist covering ${priorityWatchlist}.`
    },
    {
      title: "Forecast Outlook",
      body: `${forecasts.length} ICPAC bulletins are loaded; IPC projected Phase 3+ is >=30% in ${projectedHigh} countries, and FEWS near-term phase 3+ appears in ${fewsMl1High} countries.`
    },
    {
      title: "Source Signal Status",
      body: `This bulletin draws on GDACS, ReliefWeb, IPC, ICPAC, FEWS Data Warehouse, ${cycloneSourceStatus.checked_count ?? 0} dedicated cyclone-source checks, and Copernicus flood-source posture checks in the current refresh cycle.`
    },
    {
      title: "Conflict And Displacement",
      body: `${conflictCountries.length} countries show conflict-related reporting signals and ${displacementCountries.length} show displacement-related reporting signals in the current source window.`
    },
    {
      title: "Flood Exposure",
      body: `${floodCountries.length} countries show mapped flood signals from current GDACS items, led by ${floodTopCountries}. Copernicus flood posture is ${cemsFloodStatus.overall || "unknown"}.`
    }
  ];
  ...
  els.operationalEventSummary.innerHTML = `
    <ul class="bulletin-list">
      <li><strong>GDACS hazard events:</strong> ${hazards.length} ingested feed items, with mapped top-country concentration in ${gdacsTopCountries}.</li>
      <li><strong>Flood-linked countries:</strong> ${floodCountries.length}, led by ${floodTopCountries}.</li>
      <li><strong>ReliefWeb reports (30d):</strong> ${reports30d.length} current reports, led by ${reliefTopCountries}.</li>
      <li><strong>Conflict and displacement source matches:</strong> ${conflictDisplacementSignals.length} current linked items across ${overlapCountries.length} overlap countries.</li>
      <li><strong>Cyclone projection or advisory signals:</strong> ${cycloneIntel.projection_signal_count || 0}, with ${cycloneIntel.active_cyclone_count || 0} active named cyclones in the AFRO-relevant event stream${(cycloneIntel.active_cyclone_count || 0) ? ` across ${activeCycloneLocations}` : "; current signals are advisory only"}.</li>
  ...
  renderRecommendationList(
    ...
    "No recommendations are available for this refresh."
  );
  ...
  els.operationalSourceDetail.innerHTML = `
    ...
    <p><strong>IOM DTM displacement feed:</strong> ${iomStatusDetail}${iomStatusError}.</p>
  ...
  const triggerRows = [
    {
      trigger: "Food Security Deterioration",
      threshold: "IPC projected Phase 3+ >= 30% in >= 3 countries OR FEWS ML1 phase >= 3 in >= 4 countries",
      ...
      action: "Convene food-security technical review within 24h; validate with country office and cluster leads."
    },
    {
      trigger: "Conflict/Displacement Escalation",
      threshold: "UNHCR matched displacement signals >= 5 in current refresh",
      current: `${unhcrMatchedTotal} matched`,
      status: unhcrMatchedTotal >= 5 ? "active" : "watch",
      action: "Initiate displacement access and service continuity check for top affected countries."
    },
    {
      trigger: "Source Coverage Pressure",
      threshold: "ACAPS crawl cap warning streak >= 3",
      current: acapsWarning || "none",
      status: acapsWarning ? "active" : "watch",
      action: "Review crawl depth configuration and source retrieval strategy before next leadership briefing."
    },
    {
      trigger: "Multi-signal Country Priority",
      threshold: "Risk >= 70 with both conflict and displacement signals",
      ...
      action: "Escalate integrated country support review (health, nutrition, and emergency operations)."
    },
    {
      trigger: "Flood-Food Security Overlap",
      threshold: "Flood signals > 0 with IPC Phase 3+ >= 20%",
      ...
      action: "Validate access disruption, pre-position food and nutrition stocks, and confirm district-level exposure before leadership escalation."
    }
  ];
  ...
  const triggerRowsMarkup = triggerRows.map((row) => `
    <tr>
      ...
      <td>${row.threshold}</td>
      <td>${row.current}</td>
      <td><span class="tag ${row.status === "active" ? "bad" : "warn"}">${row.status === "active" ? "Active" : "Watch"}</span></td>
      <td>${row.action}</td>
    </tr>
  `).join("");

  els.operationalDecisionProtocol.innerHTML = `
    <p><strong>Use:</strong> This matrix links current signal states to recommended leadership actions. Trigger status is refresh-driven and must be validated with country and partner channels before formal activation.</p>
    ...
    <p><span class="tag warn">Protocol note</span> If any trigger is Active for two consecutive refresh cycles, escalate to same-day regional leadership review and issue a country validation request.</p>
  `;
  ...
  els.operationalConflictDisplacementInsert.innerHTML = conflictInsertCountries.map((country) => {
    ...
    `;
  }).join("") || "No conflict or displacement insert content is available in this refresh.";
...
function renderCyclonePage() {
  ...
  els.cycloneSummary.innerHTML = `
    ...
    <p><strong>Projection signals:</strong> ${projectionSignals.length} projection/advisory items detected across monitored sources${projectionSignals.length && !activeCyclones.length ? ", with no active AFRO cyclone event currently documented" : ""}.</p>
  ...
  els.cycloneProjectionFeed.innerHTML = projectionSignals.length
    ? projectionSignals.slice(0, 30).map((p) => `
      <div class="feed-item">
        <a href="${p.url || "#"}" target="_blank" rel="noreferrer">${p.title || "Cyclone projection signal"}</a>
  ...
  els.activeCyclonesList.innerHTML = activeCyclones.length
    ...
    : "No active named cyclone systems were detected in the configured active window.";

  renderRecommendationList(
    els.cycloneRecommendations,
    aiRecommendations?.byIssue?.cyclone,
    "No cyclone recommendations can be generated from the current refresh."
  );
}
...
function bindEvents() {
  els.presentationModeBtn?.addEventListener("click", () => {
    ...
    els.presentationModeBtn.textContent = isPresentationMode ? "Disable Presentation Mode" : "Enable Presentation Mode";
  ...
  els.exportSnapshotBtn?.addEventListener("click", async () => {
    ...
    const originalLabel = els.exportSnapshotBtn.textContent;
    els.exportSnapshotBtn.disabled = true;
    els.exportSnapshotBtn.textContent = "Preparing export...";
    showExportStatus("Preparing export. Checking chart readiness...", "info", 0);

    const readiness = await waitForExportChartsReady(15000);
    if (!readiness?.ok) {
      const reason = readiness?.reason || "unknown";
      els.exportSnapshotBtn.disabled = false;
      els.exportSnapshotBtn.textContent = originalLabel;
    ...
    requestAnimationFrame(() => {
      window.print();
      printRestoreTimer = setTimeout(() => {
        restoreOperationalBulletinPrint();
      }, 750);
      els.exportSnapshotBtn.disabled = false;
      els.exportSnapshotBtn.textContent = originalLabel;
      showExportStatus("Export sent to print dialog.", "success", 3500);
    });
  });

  els.exportValidationBtn?.addEventListener("click", async () => {
    stopAutoRotate("validation snapshot export");
    const originalLabel = els.exportValidationBtn.textContent;
    els.exportValidationBtn.disabled = true;
    els.exportValidationBtn.textContent = "Exporting...";
    showExportStatus("Preparing validation snapshot...", "info", 0);
    try {
      await exportValidationSnapshot();
    } finally {
      els.exportValidationBtn.disabled = false;
      els.exportValidationBtn.textContent = originalLabel;
    }
  });
  ...
  try {
    ...
    if (!payload && forceRefresh) {
      try {
        ...
        setAutoStatus("Live sources were slow; loaded latest server snapshot to keep the dashboard responsive.");
      } catch (fallbackErr) {
        lastErr = fallbackErr;
      }
    }
    ...
    if (freshnessMode === "lenient") {
      ...
      els.staleBanner.innerHTML = `<strong>Lenient mode active:</strong> Nutrition values older than ${policy.nutrition_max_age_years ?? 3} years are shown with a <span class="tag stale">Stale</span> label. Use for context only, not operational planning.`;
    } else {
    ...
    try {
      ...
      if (cached?.payload) {
        ...
        setAutoStatus(`Showing cached data from ${cachedAt}; retry refresh for latest live pull.`);
        if (!hasAutoStarted) {
          startAutoRotate();
          hasAutoStarted = true;
        }
    ...
    if (!restoredFromCache) {
      els.metricGrid.innerHTML = `<article class="metric-card"><h3>Load Error</h3><div class="metric-note">${err.message}</div></article>`;
      setAutoStatus("Auto display is off.");
    }
  } finally {
    if (els.refreshBtn) {
      els.refreshBtn.disabled = false;
      els.refreshBtn.textContent = "Update Dashboard";
    }
  }
}
