...
async function fetchCemsFloodSourceStatus() {
  ...
  const pages = [
    { key: "data_access", source: "CEMS Flood Data Access", url: CEMS_FLOOD_DATA_ACCESS_URL, type: "documentation" },
    { key: "products", source: "Copernicus Global Flood Products", url: CEMS_FLOOD_PRODUCTS_URL, type: "documentation" },
    { key: "gfm_wiki", source: "Global Flood Monitoring Wiki", url: GFM_WIKI_URL, type: "documentation" },
    { key: "gfm_portal", source: "Global Flood Monitoring Portal", url: GFM_PORTAL_URL, type: "portal" }
  ];
  ...
  const sources = await Promise.all(pages.map(async (page) => {
    try {
      const response = await axios.get(page.url, { timeout: 20000, headers });
      const $ = cheerio.load(response.data || "");
      ...
      return {
        ...
        summary: page.type === "portal"
          ? (requiresLogin ? "Portal reachable, but live flood portal access is credentialed." : "Portal reachable without explicit credential prompt.")
...
function cycloneIntelligence(countryMap, hazards, reports, forecasts, dedicatedCycloneSignals, dedicatedSourceStatus) {
  ...
  const activeEvents = cycloneEvents.filter((e) => {
  ...
  const activeCyclones = Object.values(grouped)
    .map((g) => ({
      name: g.name,
...
app.get("/api/dashboard-data", async (req, res) => {
  try {
    ...
    const [hazards, reportsBundle, unhcrReports, idmcReports, unochaReports, iomDtmBundle, forecasts, ipc_source_status, dedicatedCycloneBundle, acledBundle, fewsBundle, fewsIpcBundle, acapsBundle, whoDonBundle, cemsFloodSourceStatus] = await Promise.all([
    ...
    const responsePayload = {
      ...
      data_notes: [
        ...
        "Regional flood signals are also extracted from ReliefWeb RSS flood-like items with AFRO context keywords for broader situational awareness.",
        "Copernicus CEMS Flood / GloFAS documentation is monitored as a hydrological flood-source posture check; this publication bundle does not claim direct live forecast extraction when portal access is credentialed.",
        ...
        "Cyclone projection signals are monitored from ICPAC forecast bulletins and ReliefWeb advisory-style reports.",
        ...
        "Active cyclone events are documented from GDACS event stream and grouped by cyclone name when available.",
